...
mod struct_tests {
    use crate::{
        from_slice, from_value, to_value, to_vec, value_from_slice, value_to_vec, HashableValue,
        SerOptions, Value,
    };
    use serde::{de, ser};
...
mod value_tests {
    ...
    fn qc_roundtrip() {
        fn roundtrip(original: Value) {
            ...
            let _tripped = value_from_slice(&vec, Default::default()).unwrap();
            // TODO: The following assertion fails for collections, the equality
            // checking needs to be fixed.
